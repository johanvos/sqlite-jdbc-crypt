...
public class SQLiteConfig {
    ...
    public static enum Pragma {
        ...
        FAST_KDF_ITER("fast_kdf_iter", "Allows to increase or decrease the number of iterations used", null),
        HMAC_USE("hmac_use", "Allows to enable or disable the use of per-page HMACs", null),
        HMAC_PGNO("hmac_pgno", "Allows to modify in which endianess the page number should be used", null),
        HMAC_SALT_MASK("hmac_salt_mask", "Allows to modify the mask byte when calculating HMAC", null),
        KDF_ALGORITHM("kdf_algorithm", "Allows to modify hash algorithm used for key derivation", null),
        HMAC_ALGORITHM("hmac_algorithm", "Allows to modify hash algorithm used for HMAC calculation", null),
        PLAINTEXT_HEADER_SIZE("plaintext_header_size", "Allows to configure the cipher scheme to keep the database header partially unencrypted", null),
        MC_USE_SQL_INTERFACE("mc_use_sql_interface", "Set if the SQL interface for SQLiteMC commands must be used", OnOff);
