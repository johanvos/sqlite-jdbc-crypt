...
public class SQLiteMCURIInterfaceTest {
    ...
    public Connection cipherDatabaseOpen(String dbPath, String key) throws SQLException {
        try {
            return DriverManager.getConnection("jdbc:sqlite:file:" + dbPath + "?cipher=sqlcipher&legacy=1&kdf_iter=4000&key=" + key);
    ...
    public void genericDatabaseTest(SQLiteMCConfig config) throws IOException, SQLException {
        ...
        assertTrue(databaseIsReadable(c), String.format("1. Be sure the database with config %s can be read with the key '%s'", config.getClass().getSimpleName(), Key1));
        c.close();

        //3. Ensure db is not readable without the good password (Using Key2 as password)
        c = cipherDatabaseOpen(path, Key2);
        assertNull(c, String.format("2 Be sure the database with config %s cannot be read with the key '%s' (good key is %s)", config.getClass().getSimpleName(), Key2, Key1));

        //4. Rekey the database
        c = cipherDatabaseOpen(path, Key1);
        assertTrue(databaseIsReadable(c), String.format("3. Be sure the database with config %s can be read before rekeying with the key '%s' (replacing %s with %s)", config.getClass().getSimpleName(), Key2, Key1, Key2));
        c.createStatement().execute(String.format("PRAGMA rekey=%s", Key2));
        assertTrue(databaseIsReadable(c), "4. Be sure the database is still readable after rekeying");
        c.close();

        //5. Should now be readable with Key2
        c = cipherDatabaseOpen(path, Key2);
        assertTrue(databaseIsReadable(c), String.format("5. Should now be able to open the database with config %s and the new key '%s'", config.getClass().getSimpleName(), Key2));
        c.close();
    }
    ...
    public void sqlCipherDatabaseHexKeyTest() throws SQLException, IOException {
        ...
        assertTrue(databaseIsReadable(c), String.format("1. Be sure the database with config %s can be read with the key '%s'", "SQLCipher", Key1));
        c.close();

        //3. Ensure not readable with wrong key
        Connection c2 = cipherDatabaseOpen(dbfile, Key2);
        assertFalse(databaseIsReadable(c2), String.format("2. Be sure the database with config %s cannot be read with the key '%s'", "SQLCipher", Key2));
        c.close();
    ...
    public void sqlCipherDatabaseSpecialKeyTest() throws SQLException, IOException {
        ...
        assertTrue(databaseIsReadable(c), "2. Be sure the database is readable using PRAGMA method and key containing special characters");
        c.close();
        ...
        assertTrue(databaseIsReadable(c), "3. Be sure the database is readable using SQL method and key containing special characters");
        c.close();
    }
